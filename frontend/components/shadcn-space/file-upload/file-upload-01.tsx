...
export const FileUploadStruc: React.FC<FileUploadProps> = ({ onChange }) => {
  ...
  const handleFileChange = (newFiles: File[]) => {
    // Take only the first file from the incoming array
    const singleFile = newFiles.slice(0, 1);

    setFiles(singleFile); // Overwrite state instead of appending
    onChange?.(singleFile); // Pass only one file to the parent
  };
      ...
      >
        <input
          ...
          onChange={(e) => {
            ...
            handleFileChange(files.slice(0, 1)); // Only pass the first file
          }}
          className="hidden"
        />
...
const FileItem: React.FC<FileItemProps> = ({
  ...
  <motion.div
    ...
    className={cn(
      "relative overflow-hidden z-40 bg-card border flex flex-col items-start justify-start md:h-24 p-4 w-full mx-auto rounded-md shadow-sm",
    )}
  >
